...
export default function SingleQuizAttemptPage({
  params,
}: {
  ...
  useEffect(() => {
    ...
    fetch(`/api/attempt/${attemptId}`, {
      ...
      .catch((err) => {
        setError(err.message || "Network error loading attempt");
        setIsLoading(false);
      });
  }, [attemptId, router]);
  ...
  const handleSelectOption = async (option: string) => {
    ...
    if (!currentQ || isChecking) return;

    // Prevent re-selection if already evaluated
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!isCorrect) {
          // Spaced Repetition: Re-queue question 2-3 questions ahead
        ...
        // Direct auto-advance to next question:
        // 1.2s for correct, 2.2s for incorrect so student can review explanation
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      setError(err.message || "Submission network error");
      setIsSubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center space-y-4 bg-card border border-border p-8 rounded-2xl shadow-lg">
          ...
          <p className="text-sm text-muted-foreground">{error || "No questions found for this quiz."}</p>
          <button
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#070D1B] flex flex-col transition-colors">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground truncate">
                {courseName} {currentQuestion.topic?.name ? `• ${currentQuestion.topic.name}` : ""}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            {expiresAt && (
              <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-700 dark:text-amber-400 font-mono font-bold text-sm">
                ...
                <TimerDisplay expiresAt={expiresAt} onExpire={handleTimerExpire} />
              </div>
            )}
            <button
              type="button"
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        ...
        <div className="bg-white dark:bg-[#0E1A38] border border-border/80 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6 transition-all">
          ...
          {/* Header row: Question X of Y + Memory Reinforcement Badge + Difficulty */}
                ...
                <div className="space-y-1.5 flex-1 min-w-0">
                  ...
                  {!currentFeedback.isCorrect && (
                    <p className="text-[11px] text-amber-800 dark:text-amber-300 font-medium pt-1">
                      🔁 <strong>Learning Mode:</strong> We've re-queued this question so it appears again a few questions later. That way, you memorize and master it before finishing!
                    </p>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4">
            <div className="flex items-center gap-3 text-red-500">
              <AlertTriangle className="w-6 h-6" />
              <h3 className="text-lg font-bold text-foreground">Leave Assessment?</h3>
            </div>
            <p className="text-sm text-muted-foreground">
              Are you sure you want to leave? Your progress will not be finalized until submitted.
            </p>
